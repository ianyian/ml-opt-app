...
parameter_criterion = st.sidebar.select_slider(
    'Performance measure (criterion)', options=['poisson', 'absolute_error', 'squared_error', 'friedman_mse'])
...
def build_model(df):
    ...
    grid_pivot = grid_reset.pivot(
        index='max_features', columns='n_estimators', values='R2')
